# 🌐 Personal Portfolio Website

A responsive personal portfolio website created using **HTML** and **CSS** to showcase my profile, education, skills, projects, resume, and contact information.

## 📌 Overview

This project was developed as part of my web development learning journey and demonstrates my understanding of front-end development fundamentals. The website provides a simple and user-friendly interface to present my academic background, technical skills, and projects.

## ✨ Features

* Responsive and clean portfolio layout
* Personal introduction section
* Education details in tabular format
* Skills showcase
* Project showcase with descriptions and GitHub links
* Resume download section
* Contact information section
* Smooth scrolling navigation
* Interactive hover effects and animations

## 🛠️ Technologies Used

* HTML5
* CSS3

## 📂 Project Structure

```text
CODSOFT_TASK1/
│
├── index.html
├── style.css
├── admin.jpg
├── VikashDhakad_InternshalaResume.pdf
├── Screenshot_1.png
├── Screenshot_2.png
└── Screenshot_3.png
```

## 🖼️ Sections Included

### Header

* Name and personal tagline
* Profile image

### About

* Brief introduction and career objectives

### Education

* Academic qualifications and scores

### Skills

* Technical skills and areas of expertise

### Projects

* Resume Website
* Secure Chat Application

### Resume

* Downloadable PDF resume

### Contact

* Email
* Phone
* GitHub Profile
* LinkedIn Profile

## 📸 Project Preview

Screenshots of the website are included in this repository.

## 🔗 GitHub Repository

Repository Link:
https://github.com/vikashdev4/CODSOFT_TASK1

## 🚀 Live Demo
Live Website:
https://vikashdev4.github.io/CODSOFT_TASK1/

## 👨‍💻 Author

**Vikash Dhakad**

GitHub: https://github.com/vikashdev4

LinkedIn: https://www.linkedin.com/in/vikashdev4/

## 📄 License

This project is created for educational and learning purposes.

---

⭐ Thank you for visiting this repository. Feedback and suggestions are always welcome.
